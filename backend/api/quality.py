...
def _get_architecture_notes() -> list[dict[str, str]]:
    ...
    return [
        {
            "severity": "warning",
            "title": "Synchrone save_to_pgvector() in async context",
            "description": "ingest.py aanroept een synchrone functie vanuit async code, wat de event loop kan blokkeren bij grote documenten.",
            "action": "Refactor naar asyncio.to_thread() of maak volledig async.",
        },
        {
            "severity": "warning",
            "title": "Python-side JSON aggregatie in StatsService",
            "description": "retrieved_sources wordt in Python geheugen geaggregeerd i.p.v. SQL-side. Schaalt slecht bij >10.000 logs.",
            "action": "Gebruik PostgreSQL jsonb_array_elements() voor aggregatie.",
        },
        {
            "severity": "info",
            "title": "Hardcoded chain-index toegang",
            "description": "chat.py gebruikt rag_chain['generation'].middle[1] om de LLM te benaderen. Fragiel bij LangChain upgrades.",
            "action": "Sla de LLM instantie apart op in app.state.",
        },
        {
            "severity": "info",
            "title": "datetime.utcnow() deprecated",
            "description": "models.py gebruikt datetime.utcnow() dat deprecated is in Python 3.12.",
            "action": "Vervang door datetime.now(timezone.utc).",
        },
        {
            "severity": "info",
            "title": "Geen caching op admin stats",
            "description": "Elke admin dashboard refresh herberekent alle statistieken. Bij groei zorgt dit voor onnodige DB belasting.",
            "action": "Voeg een 5-minuten cache toe via functools.lru_cache of Redis.",
        },
        {
            "severity": "ok",
            ...
            "action": "Monitoren, geen actie vereist.",
        },
    ]
