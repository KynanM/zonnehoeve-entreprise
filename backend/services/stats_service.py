...
from sqlalchemy import select, func, cast, Date, desc, DateTime
...
class StatsService:
    ...
    async def get_dashboard_stats(self, days: int = 14) -> Dict[str, Any]:
        ...
        base_stats_query = select(
            ...
            func.count(ChatLog.id).filter(ChatLog.user_feedback == 'thumbs_down').label("thumbs_down")
        )
        ...
        satisfaction_rate = round((thumbs_up / feedback_total * 100), 1) if feedback_total > 0 else None

        # 2. Activiteit per dag
        since = (datetime.now() - timedelta(days=days)).replace(tzinfo=None, microsecond=0)
        activity_query = (
            select(
                cast(ChatLog.timestamp, Date).label("day"),
        ...
        daily_activity = [{"day": str(row.day), "count": row.count} for row in activity_res.fetchall()]
        ...
        sources_query = select(ChatLog.retrieved_sources).where(ChatLog.retrieved_sources.isnot(None))
